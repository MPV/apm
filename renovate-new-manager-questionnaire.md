...
- **Copilot CLI plugin system** ([docs](https://docs.github.com/en/copilot/reference/cli-plugin-reference)) — GitHub Copilot CLI has a native plugin format with `plugin.json` manifest (searched at `.plugin/plugin.json`, `.github/plugin/plugin.json`, or `.claude-plugin/plugin.json`) and `marketplace.json` distribution. Plugin components include agents, skills, hooks, MCP servers, and LSP servers. Sources support `github`, `url`, `git-subdir`, and relative paths with `ref`/`sha` pinning. Installation via `copilot plugin install PLUGIN@MARKETPLACE` or `OWNER/REPO`. Default marketplaces: `copilot-plugins` and `awesome-copilot`. This is a distribution and runtime system, not a package manager — no transitive dependency resolution, no lock file, no multi-agent-runtime compilation. The `marketplace.json` schema is shared with Claude Code's plugin system (see below). APM auto-detects Copilot CLI plugins from all manifest locations and normalizes them into standard APM packages with full version locking and governance.
- **Claude Code plugin system** ([docs](https://code.claude.com/docs/en/plugins)) — Claude Code has its own native plugin format (`.claude-plugin/plugin.json`) with marketplace distribution (`.claude-plugin/marketplace.json`), `/plugin install` commands, and an official Anthropic marketplace (`claude-plugins-official`). Shares the same `marketplace.json` schema and nearly identical `plugin.json` manifest as Copilot CLI, but additionally supports `npm` as a plugin source type. Installation via `/plugin install PLUGIN@MARKETPLACE`. Like Copilot CLI, this is a distribution and runtime system — no transitive dependency resolution, no lock files with content-hash verification, no multi-agent-runtime compilation. APM bridges both plugin ecosystems bidirectionally: it auto-detects native plugins from either format, normalizes them into standard APM packages, and can export APM packages back to the plugin format via `apm pack --format plugin`.
